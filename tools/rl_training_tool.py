...
# Path to tinker-atropos submodule (relative to gauss-agent root)
GAUSS_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = GAUSS_ROOT / "tinker-atropos"
ENVIRONMENTS_DIR = TINKER_ATROPOS_ROOT / "tinker_atropos" / "environments"
CONFIGS_DIR = TINKER_ATROPOS_ROOT / "configs"
LOGS_DIR = Path(os.getenv("GAUSS_HOME", Path.home() / ".gauss")) / "logs" / "rl_training"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.gauss/.env",
        }, indent=2)
