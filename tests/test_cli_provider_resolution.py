...
from gauss_cli.auth import AuthError
from gauss_cli import main as gauss_main
...
def test_gauss_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in GaussCLI.__init__")

    monkeypatch.setattr("gauss_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    monkeypatch.setenv("GAUSS_INFERENCE_PROVIDER", "openrouter")
    ...
    shell = cli.GaussCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    shell = cli.GaussCLI(compact=True, max_turns=1)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.GaussCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.GaussCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("gauss_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("gauss_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("gauss_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("gauss_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(gauss_main, "_prompt_provider_choice", lambda choices: len(choices) - 1)

    gauss_main.cmd_model(SimpleNamespace())
    output = capsys.readouterr().out
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "gauss_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("gauss_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("gauss_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("gauss_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("gauss_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "gauss_cli.models.probe_api_models",
