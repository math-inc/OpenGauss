# Contributing to Gauss Agent

Thank you for contributing to Gauss Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Gauss to work everywhere.
...
Bundled skills (in `skills/`) ship with every Gauss install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `gauss skills browse` (labeled "official") and install it with `gauss skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `gauss skills install`.
...
```bash
mkdir -p ~/.gauss/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.gauss/config.yaml
touch ~/.gauss/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.gauss/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/gauss" ~/.local/bin/gauss
...
├── cli.py                    # GaussCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (gauss-cli, gauss-telegram, etc.)
├── gauss_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── gauss_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.gauss/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (gauss-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.gauss/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.gauss/`)

| Path | Purpose |
|------|---------|
| `~/.gauss/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.gauss/.env` | API keys and secrets |
| `~/.gauss/auth.json` | OAuth credentials (Nous Portal) |
| `~/.gauss/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.gauss/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.gauss/state.db` | SQLite session database |
| `~/.gauss/sessions/` | JSON session logs |
| `~/.gauss/cron/` | Scheduled job data |
| `~/.gauss/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`gauss_state.py`) with full-text search and unique session titles. JSON logs go to `~/.gauss/sessions/`.
...
Four fields are supported under `metadata.gauss`:
...
The user may skip setup and keep loading the skill. Gauss only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `gauss setup` or update `~/.gauss/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Gauss tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `gauss --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Gauss uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.gauss/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `gauss_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `gauss_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Gauss runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Gauss has terminal access. Security matters.
...
2. **Test manually**: Run `gauss` and exercise the code path you changed
