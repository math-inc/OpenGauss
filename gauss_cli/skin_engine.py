"""Gauss CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in the active Gauss home ``<home>/skins/``
(``~/.gauss`` by default, with legacy ``~/.gauss`` compatibility) or as
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Gauss"                 # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      status_glyph: "@"                  # Status bar / running-agent glyph
      goodbye: "Goodbye! @"              # Shown on exit
      response_label: " ∑ Gauss "        # Response box header label
    ...
    # Optional responsive art variants
    banner_logo_compact: |
      [bold #FFD700]GAUSS[/] [#FFF8DC]· compact logo[/]
    banner_hero_compact: |
      [#CD7F32]minimal hero art[/]
...
.. code-block:: python
    ...
    from gauss_cli.skin_engine import get_active_skin, list_skins, set_active_skin
    ...
    set_active_skin("mytheme")            # Switch to user skin from <home>/skins/
...
Drop a YAML file in ``<gauss-home>/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""            # Rich-markup ASCII art logo (wide)
    banner_logo_compact: str = ""    # Rich-markup logo used on narrower terminals
    banner_hero: str = ""            # Rich-markup hero art (wide)
    banner_hero_compact: str = ""    # Rich-markup hero art used on narrower terminals
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Gauss — gold and kawaii",
        "colors": {
...
def _build_skin_config(data: Dict[str, Any]) -> SkinConfig:
    ...
    return SkinConfig(
        ...
        banner_hero_compact=data.get("banner_hero_compact", ""),
    )
...
def get_active_status_glyph(fallback: str = "@") -> str:
    """Get the status/running glyph from the active skin."""
    try:
        glyph = get_active_skin().get_branding("status_glyph", fallback)
    except Exception:
        glyph = fallback
    glyph = str(glyph or fallback).strip()
    return glyph or fallback
...
def get_prompt_toolkit_style_overrides() -> Dict[str, str]:
    ...
    surface = skin.get_color("ui_surface", "#1a1a2e")
    surface_alt = skin.get_color("ui_surface_alt", "#333355")
    surface_text = skin.get_color("ui_text", text)
    surface_muted = skin.get_color("ui_muted", dim)
    status_bg = skin.get_color("status_bg", surface)
    status_text = skin.get_color("status_text", surface_text)
    status_strong = skin.get_color("status_strong", skin.get_color("ui_accent", title))
    status_dim = skin.get_color("status_dim", surface_muted)
    status_good = skin.get_color("status_good", skin.get_color("ui_ok", "#8FBC8F"))
    status_warn = skin.get_color("status_warn", warn)
    status_bad = skin.get_color("status_bad", warn)
    status_critical = skin.get_color("status_critical", error)
    ...
    return {
        ...
        "completion-menu": f"bg:{surface} {surface_text}",
        "completion-menu.completion": f"bg:{surface} {surface_text}",
        "completion-menu.completion.current": f"bg:{surface_alt} {skin.get_color('ui_accent', title)} bold",
        "completion-menu.meta.completion": f"bg:{surface} {surface_muted}",
        "completion-menu.meta.completion.current": f"bg:{surface_alt} {label}",
        ...
        "voice-prompt": label,
        "voice-recording": f"{error} bold",
        "voice-processing": f"{warn} italic",
        "voice-status": f"bg:{status_bg} {label}",
        "voice-status-recording": f"bg:{status_bg} {error} bold",
    }
